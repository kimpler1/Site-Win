...
export default function NewYearCostumesPage() {
  const newYearCategories: any[] = []
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold text-slate-700 mb-4">Новогодние костюмы</h1>
              <p className="text-xl text-slate-600">Волшебные костюмы для создания настоящей новогодней сказки</p>
            </div>

            {/* Удалил карточки категорий */}
            {/* <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {newYearCategories.map((category, index) => (
                <Card
                  key={index}
                  className="group hover:shadow-lg transition-all duration-300 bg-white border-amber-200 h-32 overflow-hidden"
                >
                  <CardContent className="p-0 h-full relative">
                    {category.available ? (
                      <Link href="/new-year/santa-snowmaiden" className="block h-full">
                        <div
                          className="h-full relative bg-cover bg-center"
                          style={{
                            backgroundImage: `url('/new-year-winter-costume.jpg')`,
                          }}
                        >
                          <div className="absolute inset-0 bg-yellow-400/80 hover:bg-yellow-500/80 transition-colors duration-200 flex items-center justify-center">
                            <span className="text-amber-900 font-semibold text-center px-2">{category.name}</span>
                          </div>
                        </div>
                      </Link>
                    ) : (
                      ...
                      >
                        <div className="absolute inset-0 bg-gray-100/80 flex items-center justify-center">
                          <div className="text-slate-600 font-semibold text-center px-2">
                            {category.name}
              ...
              <Card className="bg-white border-amber-200">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-slate-600 mb-6">
                    Мы постоянно расширяем нашу коллекцию новогодних костюмов. Следите за обновлениями или свяжитесь с
